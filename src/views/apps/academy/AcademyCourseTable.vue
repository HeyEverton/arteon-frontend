...
const updateOptions = () => {
  // sortBy and orderBy omitted from local mock
}

const headers = [
  { title: 'Nome do Curso', key: 'courseName' },
  { title: 'Tempo', key: 'time', sortable: false },
...
const courses = computed((): Course[] => {
  ...
  if (searchQuery.value)
    filtered = filtered.filter(c => c.courseTitle.toLowerCase().includes(searchQuery.value.toLowerCase()))
    ...
    <VCardText>
      <div class="d-flex flex-wrap justify-space-between align-center gap-4">
        <h5 class="text-h5 font-weight-medium">
          Cursos que você está acompanhando
        </h5>
        <div>
          <AppTextField
            v-model="searchQuery"
